...
def get_active_namenode(name_nodes):
    for name_node in name_nodes.split(","):
        resp = requests.get(f"http://{name_node}:50070/jmx?qry=Hadoop:service=NameNode,name=NameNodeStatus")
...
class BaseTransfer:
    ...
    def impala_query_sql(self, sql, command_description='', status_exit=True):
        sql = sql.replace("\n", "").replace("'", "'\\''").strip()
        command = f"""
            impala-shell -i{self.impala_machine_host} -q '{sql}';
            """.strip()
...
class TagTransfer(BaseTransfer):
    def __init__(self):
        ...
        self.tag_base_time =None

        data_type_list = ["double", "string", "string", "bigint", "bigint", "bigint", "string"]
        ...
        try:
            ...
            if not project_id:
                raise ConfigurationError(f"The project: {self.project_name} does not exist")
            project_id = project_id[0]
            sql = f"""
                    select t.id as id, t.name as name, p.status as partition_status
    ...
    def transfer_data(self):
        if self.transfer_cross_environment == "true":
            # 数据本地落盘
            command = f"""
                       hadoop fs -get {self.source_hdfs_cache_path}/{self.database}  {self.source_hdfs_local_path}
                       """
            self.exec_command(command, "TagTransfer get_data_to_local_path ")
            # ssh scp 文件传输到target cluster 指定文件目录
            command = f"""
                        scp -r {self.source_hdfs_local_path} {self.target_ssh_trust_user}@{self.target_cluster_hdfs_host}:{self.target_hdfs_local_path}
                        """
            self.exec_command(command, "TagTransfer scp_data_to_target_cluster")
            # target cluster 存入导入文件
            command = f"""
                        hadoop fs -put -f  {self.target_hdfs_local_path}  {self.target_hdfs_cache_path}/{self.database}
                        """
            ...
            # 删除source磁盘数据
            command = f"""
                        rm -rf {self.source_hdfs_local_path}/*
                     """
            self.exec_command(command, "TagTransfer delete_date_from_source_local_path")
            # 删除target 磁盘数据
            command = f"""
                        rm -rf {self.target_hdfs_local_path}/*        
                      """
            self.exec_command(command, "TagTransfer delete_date_from_target_local_path",
                              ssh_ip="%s@%s" % (self.target_ssh_trust_user, self.target_cluster_hdfs_host))

            # # 移除impala 临时表数据
            # command = f"""
            #              hdfs dfs -rm -r {self.source_hdfs_cache_path}/{self.database}/tag/date={self.day}
            #         """
            # self.exec_command(command, "TagTransfer delete impala tmp data")
        else:
            command = f"""
    ...
    def transfer_data_history(self):
        if self.transfer_cross_environment == "true":
            for tag_name in self.tag_table_name_list:
                # 创建文件
                command = f"""
                                mkdir   {self.source_hdfs_local_path}/tag;
                                mkdir   {self.source_hdfs_local_path}/tag/history;
                                mkdir   {self.source_hdfs_local_path}/tag/history/user_tag_{tag_name};
                                mkdir   {self.source_hdfs_local_path}/tag/history/user_tag_{tag_name}/base_time={self.tag_base_time};
                          """
                self.exec_command(command, "tag Transfer create source_hdfs_local_path")
                # 数据本地落盘
                command = f"""
                              hadoop fs -get {self.source_hdfs_cache_path}/{self.database}/tag/user_tag_{tag_name}/base_time={self.tag_base_time}  {self.source_hdfs_local_path}/tag/history/user_tag_{tag_name}/base_time={self.tag_base_time}
                              """
                self.exec_command(command, "TagTransfer get_data_to_local_path")
            # 创建文件
            command = f"""
                                mkdir   {self.target_hdfs_local_path}/tag;
                                mkdir   {self.target_hdfs_local_path}/tag/history;
                        """
            self.exec_command(command, "tag Transfer mkdir",
                              ssh_ip="%s@%s" % (self.target_ssh_trust_user, self.target_cluster_hdfs_host))
            # ssh scp 文件传输到target cluster 指定文件目录
            command = f"""
                           scp -r {self.source_hdfs_local_path}/tag/history {self.target_ssh_trust_user}@{self.target_cluster_hdfs_host}:{self.target_hdfs_local_path}/tag/history
                           """
            self.exec_command(command, "TagTransfer scp_data_to_target_cluster")
            # target cluster 存入导入文件
            command = f"""
                           hadoop fs -put -f  {self.target_hdfs_local_path}/tag/history  {self.target_hdfs_cache_path}/{self.database}
                           """
            self.exec_command_retry(command, "TagTransfer put_data_to_target_hdfs",
                                    ssh_ip="%s@%s" % (self.target_ssh_trust_user, self.target_cluster_hdfs_host))
            # 删除source磁盘数据
            command = f"""
                           rm -rf {self.source_hdfs_local_path}/tag/history
                        """
            self.exec_command(command, "TagTransfer delete_date_from_source_local_path")
            # 删除target 磁盘数据
            command = f"""
                           rm -rf {self.target_hdfs_local_path}/tag/history      
                         """
            self.exec_command(command, "TagTransfer delete_date_from_target_local_path",
                              ssh_ip="%s@%s" % (self.target_ssh_trust_user, self.target_cluster_hdfs_host))
        else:
    ...
    def transfer_data_everyday(self):
        if self.transfer_cross_environment == "true":
            for tag_name in self.tag_table_name_list:
                # 创建文件
                command = f"""
                                mkdir   {self.source_hdfs_local_path}/tag;
                                mkdir   {self.source_hdfs_local_path}/tag/everyday;
                                mkdir   {self.source_hdfs_local_path}/tag/everyday/user_tag_{tag_name};
                                mkdir   {self.source_hdfs_local_path}/tag/everyday/user_tag_{tag_name}/base_time={self.tag_base_time};
                          """
                self.exec_command(command, "tag Transfer create source_hdfs_local_path")
                # 数据本地落盘
                command = f"""
                              hadoop fs -get {self.source_hdfs_cache_path}/{self.database}/tag/user_tag_{tag_name}/base_time={self.tag_base_time}  {self.source_hdfs_local_path}/tag/everyday/user_tag_{tag_name}/base_time={self.tag_base_time}
                              """
                self.exec_command(command, "TagTransfer get_data_to_local_path")
            # 创建文件
            command = f"""
                                mkdir   {self.target_hdfs_local_path}/tag;
                                mkdir   {self.target_hdfs_local_path}/tag/everyday;
                        """
            self.exec_command(command, "tag Transfer mkdir",
                              ssh_ip="%s@%s" % (self.target_ssh_trust_user, self.target_cluster_hdfs_host))
            # ssh scp 文件传输到target cluster 指定文件目录
            command = f"""
                           scp -r {self.source_hdfs_local_path}/tag/everyday {self.target_ssh_trust_user}@{self.target_cluster_hdfs_host}:{self.target_hdfs_local_path}/tag/everyday
                           """
            self.exec_command(command, "TagTransfer scp_data_to_target_cluster")
            # target cluster 存入导入文件
            command = f"""
                           hadoop fs -put -f  {self.target_hdfs_local_path}/tag/everyday  {self.target_hdfs_cache_path}/{self.database}
                           """
            self.exec_command_retry(command, "TagTransfer put_data_to_target_hdfs",
                                    ssh_ip="%s@%s" % (self.target_ssh_trust_user, self.target_cluster_hdfs_host))
            # 删除source磁盘数据
            command = f"""
                           rm -rf {self.source_hdfs_local_path}/tag/everyday
                        """
            self.exec_command(command, "TagTransfer delete_date_from_source_local_path")
            # 删除target 磁盘数据
            command = f"""
                           rm -rf {self.target_hdfs_local_path}/tag/everyday      
                         """
            self.exec_command(command, "TagTransfer delete_date_from_target_local_path",
                              ssh_ip="%s@%s" % (self.target_ssh_trust_user, self.target_cluster_hdfs_host))
        else:
...
class EventsTransfer(BaseTransfer):
    ...
    def transfer_data(self):
        if self.transfer_cross_environment == "true":
            # 创建文件
            command = f"""
                        mkdir  {self.source_hdfs_local_path}/events/date={self.day}
                     """
            self.exec_command(command, "EventsTransfer create source_hdfs_local_path")
            # source 数据本地落盘
            command = f"""
                       hadoop fs -get {self.source_hdfs_cache_path}/{self.database}/events/date={self.day}   {self.source_hdfs_local_path}/events/date={self.day}
                       """
            self.exec_command(command, "EventsTransfer get_data_to_local_path")
            # 创建文件
            command = f"""
                         mkdir   {self.target_hdfs_local_path}/events/date={self.day} 
                    """
            self.exec_command(command, "EventsTransfer mkdir",
                              ssh_ip="%s@%s" % (self.target_ssh_trust_user, self.target_cluster_hdfs_host))
            # ssh scp 文件传输到target cluster 指定文件目录
            command = f"""
                        scp -r {self.source_hdfs_local_path}/events/date={self.day} {self.target_ssh_trust_user}@{self.target_cluster_hdfs_host}:{self.target_hdfs_local_path}/events/date={self.day}
                        """
            self.exec_command(command, "EventsTransfer scp_data_to_target_cluster")

            # target cluster 存入导入文件
            command = f"""
                        hadoop fs -put -f  {self.target_hdfs_local_path}/events/date={self.day}  {self.target_hdfs_cache_path}/{self.database}/date={self.day}
                        """
            self.exec_command_retry(command, "EventsTransfer put_data_to_target_hdfs",
                                    ssh_ip="%s@%s" % (self.target_ssh_trust_user, self.target_cluster_hdfs_host))
            # 删除source磁盘数据
            command = f"""
                        rm -rf {self.source_hdfs_local_path}/events/date={self.day}
                        """
            self.exec_command(command, "EventsTransfer delete_date_from_source_local_path")
            # 删除target 磁盘数据
            command = f"""
                        rm -rf {self.target_hdfs_local_path}/events/date={self.day}          
                    """
...
def process_transfer(begin_time: datetime.date, end_time: datetime.date, transfer_type):
    ...
    for i in range(days):
        ...
        if base_transfer.transfer_tag_flag == "true":
            ...
            # if transfer_type != "history":
            #     tag_transfer.transfer_data_everyday()
            # else:
            #     tag_transfer.transfer_data_history()
